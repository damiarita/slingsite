...
export default function useCompressVideo() {
    ...
    useEffect(() => {
      if (window.Worker) {
        workerRef.current = new Worker(new URL('../app/workers/video-compressor.ts', import.meta.url));
    ...
    return workerRef.current===null? null : function (file: File, format:Format, mediaSize:MediaDimensions): Promise<File> {
      ...
      if(processorBusy){
        throw new Error("Processor is busy. Please wait for the current task to finish.");
      }
